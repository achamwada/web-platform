...
const generateContentModel = () => {
    ...
    async function contentTypeExists(contentTypeName: string): Promise<boolean> {
        try {
            const response = await client.contenttypes.contentTypesList();
    ...
    async function createOptimizelyContentType(
        ...
        try {
            await client.contenttypes.contentTypesCreate(contentType as any);
            console.log(
...
export default generateContentModel